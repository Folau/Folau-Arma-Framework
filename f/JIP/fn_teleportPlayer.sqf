...
if (count (playableUnits + switchableUnits) < 2) exitWith {
	systemChat format["[JIP] Failed - No other units are present!",_refUnit];
};
...
if (vehicle _refUnit != _refUnit) then {	// Member is in vehicle	
	...
	if (_pos isEqualTo []) then { 
		if (isTouchingGround _refUnit) then {
			player setPos (getPosATL _refUnit modelToWorld [0,-0.1,0]); 
			systemChat format["[JIP] Moved behind %1 (%2) - Empty position not found.",name _refUnit, groupId (group _refUnit), round (player distance2D _refUnit)];
		} else {
		...
		player setPos _pos;
		systemChat format["[JIP] Moved %3m from %1 (%2) - Empty position.",name _refUnit, groupId (group _refUnit), round (player distance2D _refUnit)];
	};
