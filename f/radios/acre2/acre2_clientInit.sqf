...
if (_spokenLanguages isEqualTo []) then {
	_spokenLanguages = missionNamespace getVariable [format["f_radios_settings_acre2_language_%1", playerSide],f_radios_settings_acre2_language_guer];
...
[_groupSRChannelIndex,_groupLRChannelIndex] spawn {
    ...
    if (_hasExtra) then {
		private _LRChName = ((missionNamespace getVariable [format["f_radios_settings_acre2_lr_groups_%1", playerSide],f_radios_settings_longRangeGroups]) select _LRChIndx) select 0;
		player groupChat format["[ACRE] %1: Tuned to %3.",getText (configFile >> "CfgWeapons" >> f_radios_settings_acre2_extraRadio >> "descriptionShort"), _LRChIndx + 1];
