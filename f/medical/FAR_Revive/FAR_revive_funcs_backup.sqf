FAR_Player_Actions = {
	...
	if (alive _unit && _unit isKindOf "Man") then {
		...
		FAR_addActionRevive = _unit addAction ["<t color=""#B70000"">" + "Revive" + "</t>", FAR_HandleRevive,[], 11, true, true, "", "call FAR_Check_Revive && {(cursorTarget distance player) <= 2.8}",2.8];
		FAR_addActionStabilise = _unit addAction ["<t color=""#B70000"">" + "Stabilize" + "</t>", FAR_HandleStabilize, [], 10, true, true, "", "(call FAR_Check_Stabilize) && {(cursorTarget distance player) <= 2.5}",2.5];
		...
		if (_unit getUnitTrait "Medic" || (getNumber (configFile >> "CfgVehicles" >> typeOf _unit >> "attendant") == 1)) then {
...
FAR_Check_Stabilize = {
	...
	if ((lifeState _target == "INCAPACITATED") &&
		...
		( ("FirstAidKit" in (items player)) || ("Medikit" in (items player)) || ("FirstAidKit" in (items _target)))
	) exitWith {
		true;
	};
