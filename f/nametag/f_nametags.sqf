...
if (((squadParams player) select 0) isEqualType "") then {
	if (["ZEUS",(squadParams player) select 0] call BIS_fnc_inString) then {
		F_DRAW_NAMETAGS = false;
		//player groupChat format["Name tags are disabled - Press %1 to Enable.",F_KEYNAME_NAMETAGS];
	};
};
